...
export function ReviewsList() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const result = await reviewService.getAll(OWNER_ID);
        setData(result);
      } catch (err: any) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Reviews List</h1>
        <p className="text-muted-foreground mt-2">Manage your reviews list</p>
      </div>
