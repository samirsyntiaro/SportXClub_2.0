...
const reviews = [
  {
    ...
    rating: 5,
    date: "2 days ago",
    comment: "Excellent facility with clean turf and fast booking. The trust signals make the decision easy.",
  },
  {
    name: "Priya Patel",
    rating: 5,
    date: "1 week ago",
    comment: "Very professional experience. The slot selection and review details feel much more polished now.",
  },
  {
    name: "Arjun Malhotra",
    rating: 4,
    date: "2 weeks ago",
    comment: "Great lighting and easy access. The app makes comparison and booking simple.",
  },
];
            ...
            <div className="overflow-hidden rounded-[28px] border border-white/[0.08] bg-[#101216]">
              <div className="relative aspect-[16/9] overflow-hidden">
                ...
                <div className="absolute bottom-5 left-5 max-w-xl">
                  <p className="text-sm uppercase tracking-[0.26em] text-[#6DFF3B]/80">Sports venue</p>
              ...
              <CardContent className="space-y-6 p-6 md:p-8">
                <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                  <div>
                    ...
                    <h2 className="mt-3 text-2xl  text-white">Book with confidence</h2>
                  </div>
                  <div className="rounded-[20px] border border-[#6DFF3B]/18 bg-[#6DFF3B]/10 px-4 py-3">
                    <p className="text-xs uppercase tracking-[0.22em] text-[#6DFF3B]/85">From</p>
                    <p className="mt-1 text-2xl  text-white">₹{venue.price}/hr</p>
                  </div>
                </div>

                <p className="max-w-3xl text-sm leading-7 text-white/64 md:text-base">{venue.description}</p>

                <div className="grid gap-3 md:grid-cols-3">
                  {["Real reviews", "Easy refund", "Secure payment"].map((item) => (
                    <div
                      key={item}
                    ...
                    {amenities.map((amenity) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-sm text-white/72">{amenity.label}</span>
                        </div>
                      );
                    })}
                  </div>
                ...
                <CardContent className="p-6 md:p-8">
                  ...
                  <div className="mt-5 rounded-[22px] border border-dashed border-white/[0.12] bg-[linear-gradient(135deg,rgba(109,255,59,0.08),rgba(255,255,255,0.03))] p-6">
                    <div className="flex h-52 flex-col items-center justify-center rounded-[18px] border border-white/[0.08] bg-[#050505]/70 text-center">
                      ...
                      <p className="mt-4 text-sm  text-white">Interactive map placeholder</p>
                      <p className="mt-2 max-w-sm text-xs leading-6 text-white/52">
                        Show the exact venue pin, nearby transit, and neighborhood context here.
                      </p>
                    </div>
                  </div>
              ...
              <CardContent className="p-6 md:p-8">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <h2 className="mt-3 text-2xl  text-white">Real reviews, shown clearly</h2>
                  </div>
                  <div className="text-right">
                    <p className="text-3xl  text-white">{venue.rating.toFixed(1)}</p>
                    <p className="text-sm text-white/52">{venue.reviews} verified reviews</p>
                  </div>
                </div>
                    ...
                    >
                      <div className="flex items-center justify-between gap-4">
                        <div>
                          ...
                          <p className="mt-1 text-sm text-white/52">{review.date}</p>
                        </div>
                        <div className="flex gap-1">
                          {Array.from({ length: review.rating }).map((_, index) => (
                            <Star key={index} className="h-4 w-4 fill-[#6DFF3B] text-[#6DFF3B]" />
                          ))}
                        </div>
                      </div>
                      <p className="mt-4 text-sm leading-7 text-white/66">{review.comment}</p>
                    </div>
                  ))}
                </div>
              </CardContent>
